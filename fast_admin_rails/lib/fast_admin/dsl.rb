module FastAdmin
  module DSL
    ...
    # List item fields (control what to display per record in list)
    def list_item_fields(&block)
      @list_item_fields = []
      instance_eval(&block) if block_given?
      @list_item_fields
    end

    def list_item_field(name:, label: nil, formatter: nil, order: 100)
      @list_item_fields ||= []
      @list_item_fields << {
        name: name,
        label: label,
    ...
    def list_item_fields_config
      (@list_item_fields || []).sort_by { |f| f[:order] || 100 }
    end

    # Extra routes DSL: declare collection/member routes to be auto-drawn
    def extra_routes(&block)
      @extra_routes = { collection: [], member: [] }
      instance_eval(&block) if block_given?
      @extra_routes
    end

    def collection_route(name:, method: :get)
      @extra_routes ||= { collection: [], member: [] }
      @extra_routes[:collection] << { name: name.to_sym, method: method.to_sym }
    end

    def member_route(name:, method: :get)
      @extra_routes ||= { collection: [], member: [] }
      @extra_routes[:member] << { name: name.to_sym, method: method.to_sym }
    end

    def extra_routes_config
      @extra_routes || { collection: [], member: [] }
    end
  end
end
